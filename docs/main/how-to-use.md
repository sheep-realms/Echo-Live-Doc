...
**等一下！** 如果您还没有在资源管理器中打开显示文件扩展名，请[打开显示文件扩展名](faq.md#file-extension)。特别提醒，下载完压缩包以后一定要解压！
...
2. 选中您刚刚添加的对话框源，右键，在弹出菜单中找到 “混合方式”，选择 “关闭 sRGB”。
3. 点击 OBS 菜单中的 停靠窗口 > 自定义浏览器停靠窗口，将 `editor.html` 添加到停靠窗口中。如果您不知道如何正确填写地址，同上。
4. 在弹出的编辑窗口中，根据指引填写对话数据，点击 “发送”。（注：如果您关闭了广播功能则不会有发送按钮）
5. 若要继续制作下一条对话，重复步骤 3。
...
### 方法二：通过 WebSocket 传递消息
!!! info "技术性操作"
    此方法需要您掌握一定的计算机知识，否则对您而言可能会是一场灾难，请量力而行。

1. 在浏览器中打开 `settings.html`，这是配置文件编辑器。点击 “编辑” 标签页，更改以下配置：
...
1. 在浏览器中打开 `settings.html`，这是配置文件编辑器。点击 “编辑” 标签页，更改以下配置：
    - Echo-Live → 广播 → 启用广播：关闭；
    - 消息轮询 → 启用消息轮询：开启；
    - 编辑器 → 表单预填充：
        - 启用在输出内容前插入内容：开启；
        - 启用在输出内容后插入内容：开启。
2. 配置修改完成后点击底部的 “保存” 按钮保存配置文件。配置文件是 Echo-Live 根目录中的 `config.js`，用编辑后的配置文件覆盖它。
3. 打开 OBS，将 `live.html` 作为浏览器源（勾选 “本地文件”）添加到您的场景中。
4. 选中您刚刚添加的对话框源，右键，在弹出菜单中找到 “混合方式”，选择 “关闭 sRGB”。
5. 使用文本编辑器打开 `start.js` 备用。建议您使用 [VSCode](https://code.visualstudio.com/) 等专业文本编辑器作为您的文本编辑器，不建议使用 Windows 内置的记事本。
6. 在浏览器中打开 `editor.html`，根据网页指引生成对话数据并复制。
7. 将 `start.js` 中的内容全部替换成复制的内容并保存。
8. 对话框将会自动打印修改后的消息。如果没有启用消息轮询，则需手动在 OBS 中选中浏览器源，点击 “刷新”。如果您自定义了一串消息列表，点击 “交互”，在弹出的窗口中点击任意内容区域即可开始打印下一条消息。
9. 若要继续制作下一条对话，重复步骤 3 ~ 5。当您熟悉语法格式后，您也可以直接编辑 `start.js`。


## :material-alert: 注意事项
- 对话框等前台页面基于 1920 × 1080 分辨率设计，如果您的分辨率过高或过低会影响显示效果。
...
对话框状态仪表板默认禁用，可以在[配置文件](../custom/config.md)中启用，只有在广播模式下才能生效。如果你在 OBS 中添加了多个对话框，建议启用。
...
## :material-account-box: 如何使用形象播放器

形象播放器是一个展示立绘的演出工具，于 1.6.0 版本加入，位于 Echo-Live 根目录中的 `character.html`。

1. 要使用形象播放器，首先必须打开广播模式才能使其正常工作。
2. 打开 OBS，将 `character.html` 作为浏览器源 **（不要勾选 “本地文件”）** 添加到您的场景中。如果您不知道如何正确填写地址，请直接使用浏览器打开 `character.html`，然后复制地址栏里的地址。
3. 如果您已完成了上文中描述的广播模式下应该做的准备工作，那么历史记录可以开始使用了。

形象播放器目前仍是一个实验性功能，没有可供直接操作的用户界面，仅供特殊演出使用。

## :material-link: 如何使用 URL 参数 { id="use-url-params" }

参阅 [URL 参数](../reference/url-params.md)。
