...
## 场景属性 { id="scene-attribute" }

<span class="feature-tag" title="最早可用版本" markdown>
    <span class="icon">:material-tag:</span>
    <span class="text">1.6.2</span>
</span>

!!! note "注意"

    要使用此功能，对应的浏览器源的页面权限应至少为 “用户信息的读取权限”。

通过在 OBS 的场景名称中添加后缀，可以设置场景属性，以便于在不同的场景中使用不同的样式。

语法格式为：`[属性名=属性值]`

有多个属性时：`[属性名1=属性值1,属性名2=属性值2,属性名3=属性值3]`

不包含属性值的 `[属性名]` 等价于 `[属性名=属性名]`

部分主题（如原版和玻璃）支持迷你尺寸，在场景名称中添加 `[mini]` 后缀即可应用迷你尺寸，适用于直播 PK、连麦时的画幅。

示例：`场景名称 [color=dark,mini]`，在使用原版主题的情况下，这会使其使用深色主题变体，并启用迷你尺寸。
...
!!! info "过时的内容"
    ...
    从 1.6.2 版本开始，以下预制主题已通过主题变体的形式内置，如需使用以下预制主题请直接更改主题变体。
